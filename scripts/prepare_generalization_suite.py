...
def conditional_parameter_count(kernels: int, degree: int, variant: str) -> int:
    terms = degree + 1
    per_term = 4 + 4 + 1 + (1 if variant == "xvx" else 0)
    return int(kernels * terms * per_term)
...
def main() -> None:
    ...
    for holdout in holdouts:
        ...
        for kernels in capacities:
            parameter_count = conditional_parameter_count(kernels, degree, variant)
            if holdout_moment_path is not None:
            ...
            for seed in seeds:
                for objective in objectives:
                    run_name = f"conditional_holdout-{holdout}_N{kernels}_s{seed}_{objective}"
                    cfg = {
                        "run_name": run_name,
                        ...
                        "evaluation": {"moment_keys": ["rho", "ux", "T", "qx", "sig", "M300", "M400"]},
                    }
