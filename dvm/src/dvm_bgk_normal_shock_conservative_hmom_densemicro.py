...
def main():
    ...
    try:
        ...
        save_micro_anchors_from_locals(
            _lv,
            out=_micro_out,
            x_targets=(-8.0, -6.0, -4.0, -3.0, -2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0, 8.0),
            n_micro_per_x=512,
        )
    ...
    # -------------------------------------------------------------------------
    # Save full DVM state once, so future micro-anchor selections do not require
    # rerunning the whole DVM calculation.
    # -------------------------------------------------------------------------
    try:
        def _np_cpu(a):
        ...
        _full_out = str(args.out).replace(".npz", "_fullstate.npz")

        np.savez(
            _full_out,
            x=_np_cpu(x),
            x_scaled=_np_cpu(x_scaled),
            x_mfp=_np_cpu(x_mfp),
            f=_np_cpu(f),
            v=_np_cpu(v),
            w=_np_cpu(w),
            ...
            ux=_np_cpu(ux),
            T=_np_cpu(T),
            qx=_np_cpu(extra.get("qx_neq_discrete", qx)),
            qx_raw=_np_cpu(qx),
            sig=_np_cpu(extra.get("sig_neq_discrete", sig)),
            sig_raw=_np_cpu(sig),
            ...
            **(
                {
                    ...
                    "vx_weights": axis_weights[0], "vy_weights": axis_weights[1], "vz_weights": axis_weights[2],
                }
                if axis_nodes is not None else {}
            ),
        )
        print(f"[fullstate saved] {_full_out}", flush=True)
    except Exception as e:
